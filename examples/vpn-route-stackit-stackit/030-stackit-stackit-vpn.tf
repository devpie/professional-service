...
# Gateway 1 (vpn-sna-01) — route-based: no bgp block, routes defined on the connection
...
# Gateway 2 (vpn-sna-02) — route-based: no bgp block, routes defined on the connection
...
# static_routes advertises which CIDRs are reachable through this tunnel —
# mandatory for ROUTE_BASED gateways, replacing the traffic selectors used
# by POLICY_BASED. No bgp or peering blocks in the tunnels.
